...
def main(override_config: OmegaConf):
    ...
    if NoDR:
        for key in config.env.config.domain_rand:
            if type(config.env.config.domain_rand[key]) == bool:
                config.env.config.domain_rand[key] = False
            print(key, config.env.config.domain_rand[key])
        config.env.config.domain_rand['randomize_ctrl_delay'] = True
        # config.env.config.domain_rand['ctrl_delay_step_range'] = [0,0]
        ...
    ...
    # config.env.config.robot.motion.motion_lib_type = "origin"
