    ...
    def load_motions(self, 
        ...
        class FakeCat:
            def __init__(self, element, length):
                ...
                self._length = length
                self._dim0 = element.shape[0]
            def __getitem__(self, idx: torch.Tensor):
                if isinstance(idx, torch.Tensor):
                    return self._element[idx-self._dim0*torch.arange(self._length).to(idx.device)]
                else:
                    return self[idx[0]]
            def __len__(self):
                return self._length
            @property
            def shape(self):
                element_shape = self._element.shape
                return torch.Size((self._length * element_shape[0], *element_shape[1:]))
        ...
        self._motion_aa = FakeCat(torch.tensor(_motion_aa[0]).float().to(self._device), len(_motion_aa))
        ...
        if self.has_action:
            ...
            self._motion_actions = FakeCat(_motion_actions[0].float().to(self._device), len(_motion_actions))
        if self.has_contact_mask:
            self._motion_contact_masks = FakeCat(_motion_contact_masks[0].float().to(self._device), len(_motion_contact_masks))
        self._num_motions = len(motions)
        # breakpoint()
        # self.gts = torch.cat([m.global_translation for m in motions], dim=0).float().to(self._device)
        self.gts = FakeCat(motions[0].global_translation.float().to(self._device), len(motions))
        # breakpoint()
        self.grs = FakeCat(motions[0].global_rotation.float().to(self._device), len(motions))
        self.lrs = FakeCat(motions[0].local_rotation.float().to(self._device), len(motions))
        self.grvs = FakeCat(motions[0].global_root_velocity.float().to(self._device), len(motions))
        self.gravs = FakeCat(motions[0].global_root_angular_velocity.float().to(self._device), len(motions))
        self.gavs = FakeCat(motions[0].global_angular_velocity.float().to(self._device), len(motions))
        self.gvs = FakeCat(motions[0].global_velocity.float().to(self._device), len(motions))
        self.dvs = FakeCat(motions[0].dof_vels.float().to(self._device), len(motions))
        
        if "global_translation_extend" in motions[0].__dict__:
            self.gts_t = FakeCat(motions[0].global_translation_extend.float().to(self._device), len(motions))
            self.grs_t = FakeCat(motions[0].global_rotation_extend.float().to(self._device), len(motions))
            self.gvs_t = FakeCat(motions[0].global_velocity_extend.float().to(self._device), len(motions))
            self.gavs_t = FakeCat(motions[0].global_angular_velocity_extend.float().to(self._device), len(motions))
        
        if "dof_pos" in motions[0].__dict__:
            self.dof_pos = FakeCat(motions[0].dof_pos.float().to(self._device), len(motions))
        # import ipdb; ipdb.set_trace()
        if flags.real_traj:
            self.q_qts = FakeCat(self.q_gts[0].float().to(self._device), len(self.q_gts))
            # self.q_gts = torch.cat(self.q_gts, dim=0).float().to(self._device)
            self.q_grs = FakeCat(self.q_grs[0].float().to(self._device), len(self.q_grs))
            self.q_gavs = FakeCat(self.q_gavs[0].float().to(self._device), len(self.q_gavs))
            self.q_gvs = FakeCat(self.q_gvs[0].float().to(self._device), len(self.q_gvs))
...
class MotionLibRobotWJX(MotionLibBase):
    def __init__(self, motion_lib_cfg, num_envs, device):
