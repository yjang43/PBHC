...
def save_npz(pred, save_path):
    path1, path2, path3 = f'{save_path}'.split('/')[0:3]
    trans = pred['transl']
    body_pose = torch.cat((pred['global_orient'], pred['body_pose']), dim=1)

    transform1 = sRot.from_euler('xyz', np.array([np.pi / 2, 0, np.pi]), degrees=False)
    new_root = (transform1 * sRot.from_rotvec(body_pose[:, :3])).as_rotvec()
    #
    body_pose[:, :3] = torch.from_numpy(new_root)
    trans = trans @ (torch.tensor(transform1.as_matrix().T).float())

    np.savez(f'{path1}/{path2}/{path3}/smpl.npz', betas=pred['betas'][0], gender='neutral', poses=body_pose,
             trans=trans, mocap_framerate=30.0)
